...
describe('mergeArrays', () => {
  it('should merge and sort typical input', () => {
    ...
    expect(mergeArrays(collection1, collection2, collection3)).toEqual(expected);
  });
  ...
  it('should correctly merge and sort three sorted arrays of size 50', () => {
    ...
    const collection1 = Array.from({ length: 100 }, () => Math.floor(Math.random() * 1000)).sort((a, b) => a - b); // Ascending
    const collection2 = Array.from({ length: 100 }, () => Math.floor(Math.random() * 1000)).sort((a, b) => b - a); // Descending
    const collection3 = Array.from({ length: 100 }, () => Math.floor(Math.random() * 1000)).sort((a, b) => a - b); // Ascending
